...
func (_q *TaskQuery) QueryMission() *MissionQuery {
...
// QueryTasklist chains the current query on the "tasklist" edge.
func (_q *TaskQuery) QueryTasklist() *TaskListQuery {
...
func (_q *TaskQuery) QueryPlan() *PlanQuery {
...
func (_q *TaskQuery) Clone() *TaskQuery {
	...
	return &TaskQuery{
		...
		withMission:  _q.withMission.Clone(),
		withTasklist: _q.withTasklist.Clone(),
...
func (_q *TaskQuery) WithMission(opts ...func(*MissionQuery)) *TaskQuery {
	query := (&MissionClient{config: _q.config}).Query()
...
// WithTasklist tells the query-builder to eager-load the nodes that are connected to
// the "tasklist" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *TaskQuery) WithTasklist(opts ...func(*TaskListQuery)) *TaskQuery {
	query := (&TaskListClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TaskQuery) WithPlan(opts ...func(*PlanQuery)) *TaskQuery {
	query := (&PlanClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := _q.withMission; query != nil {
		if err := _q.loadMission(ctx, query, nodes, nil,
			func(n *Task, e *Mission) { n.Edges.Mission = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withTasklist; query != nil {
		if err := _q.loadTasklist(ctx, query, nodes, nil,
			func(n *Task, e *TaskList) { n.Edges.Tasklist = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withPlan; query != nil {
		if err := _q.loadPlan(ctx, query, nodes, nil,
			func(n *Task, e *Plan) { n.Edges.Plan = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *TaskQuery) loadMission(ctx context.Context, query *MissionQuery, nodes []*Task, init func(*Task), assign func(*Task, *Mission)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Task)
	for i := range nodes {
		if nodes[i].MissionID == nil {
			continue
		}
...
func (_q *TaskQuery) loadTasklist(ctx context.Context, query *TaskListQuery, nodes []*Task, init func(*Task), assign func(*Task, *TaskList)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Task)
	for i := range nodes {
		if nodes[i].TasklistID == nil {
			continue
		}
		fk := *nodes[i].TasklistID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "tasklist_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TaskQuery) loadPlan(ctx context.Context, query *PlanQuery, nodes []*Task, init func(*Task), assign func(*Task, *Plan)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Task)
	for i := range nodes {
...
func (_q *TaskQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withMission != nil {
			_spec.Node.AddColumnOnce(task.FieldMissionID)
		}
		if _q.withTasklist != nil {
			_spec.Node.AddColumnOnce(task.FieldTasklistID)
		}
		if _q.withPlan != nil {
			_spec.Node.AddColumnOnce(task.FieldPlanID)
		}
	}
