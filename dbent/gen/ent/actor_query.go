...
type ActorQuery struct {
	...
	withCreatedMemories        *MemoryQuery
	withValidatedMemories      *MemoryQuery
	loadTotal                  []func(context.Context, []*Actor) error
	modifiers                  []func(*sql.Selector)
	withNamedCreatedMemories   map[string]*MemoryQuery
	withNamedValidatedMemories map[string]*MemoryQuery
...
// QueryCreatedMemories chains the current query on the "created_memories" edge.
func (_q *ActorQuery) QueryCreatedMemories() *MemoryQuery {
...
// QueryValidatedMemories chains the current query on the "validated_memories" edge.
func (_q *ActorQuery) QueryValidatedMemories() *MemoryQuery {
	query := (&MemoryClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *ActorQuery) Clone() *ActorQuery {
	...
	return &ActorQuery{
		...
		withCreatedMemories:   _q.withCreatedMemories.Clone(),
		withValidatedMemories: _q.withValidatedMemories.Clone(),
...
// WithCreatedMemories tells the query-builder to eager-load the nodes that are connected to
// the "created_memories" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *ActorQuery) WithCreatedMemories(opts ...func(*MemoryQuery)) *ActorQuery {
	query := (&MemoryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withCreatedMemories = query
	return _q
}

// WithValidatedMemories tells the query-builder to eager-load the nodes that are connected to
// the "validated_memories" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *ActorQuery) WithValidatedMemories(opts ...func(*MemoryQuery)) *ActorQuery {
	query := (&MemoryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withValidatedMemories = query
	return _q
}
...
func (_q *ActorQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Actor, error) {
	var (
		...
		_spec       = _q.querySpec()
		loadedTypes = [2]bool{
			_q.withCreatedMemories != nil,
			_q.withValidatedMemories != nil,
		}
	)
	...
	if query := _q.withCreatedMemories; query != nil {
		if err := _q.loadCreatedMemories(ctx, query, nodes,
			func(n *Actor) { n.Edges.CreatedMemories = []*Memory{} },
			func(n *Actor, e *Memory) { n.Edges.CreatedMemories = append(n.Edges.CreatedMemories, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withValidatedMemories; query != nil {
		if err := _q.loadValidatedMemories(ctx, query, nodes,
			func(n *Actor) { n.Edges.ValidatedMemories = []*Memory{} },
			func(n *Actor, e *Memory) { n.Edges.ValidatedMemories = append(n.Edges.ValidatedMemories, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedCreatedMemories {
		if err := _q.loadCreatedMemories(ctx, query, nodes,
			func(n *Actor) { n.appendNamedCreatedMemories(name) },
			func(n *Actor, e *Memory) { n.appendNamedCreatedMemories(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedValidatedMemories {
		if err := _q.loadValidatedMemories(ctx, query, nodes,
			func(n *Actor) { n.appendNamedValidatedMemories(name) },
			func(n *Actor, e *Memory) { n.appendNamedValidatedMemories(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *ActorQuery) loadCreatedMemories(ctx context.Context, query *MemoryQuery, nodes []*Actor, init func(*Actor), assign func(*Actor, *Memory)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Actor)
	for i := range nodes {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(memory.FieldCreatedByActorID)
	}
	query.Where(predicate.Memory(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(actor.CreatedMemoriesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.CreatedByActorID
		if fk == nil {
			return fmt.Errorf(`foreign-key "created_by_actor_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "created_by_actor_id" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *ActorQuery) loadValidatedMemories(ctx context.Context, query *MemoryQuery, nodes []*Actor, init func(*Actor), assign func(*Actor, *Memory)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Actor)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(memory.FieldValidatedByActorID)
	}
	query.Where(predicate.Memory(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(actor.ValidatedMemoriesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.ValidatedByActorID
		if fk == nil {
			return fmt.Errorf(`foreign-key "validated_by_actor_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "validated_by_actor_id" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *ActorQuery) Modify(modifiers ...func(s *sql.Selector)) *ActorSelect {
	_q.modifiers = append(_q.modifiers, modifiers...)
	return _q.Select()
}

// WithNamedCreatedMemories tells the query-builder to eager-load the nodes that are connected to the "created_memories"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *ActorQuery) WithNamedCreatedMemories(name string, opts ...func(*MemoryQuery)) *ActorQuery {
	query := (&MemoryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if _q.withNamedCreatedMemories == nil {
		_q.withNamedCreatedMemories = make(map[string]*MemoryQuery)
	}
	_q.withNamedCreatedMemories[name] = query
	return _q
}

// WithNamedValidatedMemories tells the query-builder to eager-load the nodes that are connected to the "validated_memories"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *ActorQuery) WithNamedValidatedMemories(name string, opts ...func(*MemoryQuery)) *ActorQuery {
	query := (&MemoryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if _q.withNamedValidatedMemories == nil {
		_q.withNamedValidatedMemories = make(map[string]*MemoryQuery)
	}
	_q.withNamedValidatedMemories[name] = query
	return _q
}
...
// Modify adds a query modifier for attaching custom logic to queries.
func (_s *ActorSelect) Modify(modifiers ...func(s *sql.Selector)) *ActorSelect {
