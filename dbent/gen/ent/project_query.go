...
type ProjectQuery struct {
	...
	withNamedRepos    map[string]*RepoQuery
...
func (_q *ProjectQuery) QueryRepos() *RepoQuery {
...
func (_q *ProjectQuery) QueryMemories() *MemoryQuery {
...
func (_q *ProjectQuery) WithRepos(opts ...func(*RepoQuery)) *ProjectQuery {
	query := (&RepoClient{config: _q.config}).Query()
...
func (_q *ProjectQuery) WithMemories(opts ...func(*MemoryQuery)) *ProjectQuery {
	query := (&MemoryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := _q.withRepos; query != nil {
		if err := _q.loadRepos(ctx, query, nodes,
			func(n *Project) { n.Edges.Repos = []*Repo{} },
			func(n *Project, e *Repo) { n.Edges.Repos = append(n.Edges.Repos, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withMemories; query != nil {
		if err := _q.loadMemories(ctx, query, nodes,
			func(n *Project) { n.Edges.Memories = []*Memory{} },
			func(n *Project, e *Memory) { n.Edges.Memories = append(n.Edges.Memories, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedRepos {
		if err := _q.loadRepos(ctx, query, nodes,
			func(n *Project) { n.appendNamedRepos(name) },
			func(n *Project, e *Repo) { n.appendNamedRepos(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedMemories {
		if err := _q.loadMemories(ctx, query, nodes,
			func(n *Project) { n.appendNamedMemories(name) },
			func(n *Project, e *Memory) { n.appendNamedMemories(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *ProjectQuery) loadRepos(ctx context.Context, query *RepoQuery, nodes []*Project, init func(*Project), assign func(*Project, *Repo)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(repo.FieldProjectID)
	}
...
func (_q *ProjectQuery) loadMemories(ctx context.Context, query *MemoryQuery, nodes []*Project, init func(*Project), assign func(*Project, *Memory)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(memory.FieldProjectID)
	}
	query.Where(predicate.Memory(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.MemoriesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
// WithNamedRepos tells the query-builder to eager-load the nodes that are connected to the "repos"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *ProjectQuery) WithNamedRepos(name string, opts ...func(*RepoQuery)) *ProjectQuery {
	query := (&RepoClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if _q.withNamedRepos == nil {
		_q.withNamedRepos = make(map[string]*RepoQuery)
	}
	_q.withNamedRepos[name] = query
	return _q
}

// WithNamedMemories tells the query-builder to eager-load the nodes that are connected to the "memories"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *ProjectQuery) WithNamedMemories(name string, opts ...func(*MemoryQuery)) *ProjectQuery {
	query := (&MemoryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
