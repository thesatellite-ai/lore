...
type Rule struct {
	...
	Activation rule.Activation `json:"activation,omitempty"`
	// Globs holds the value of the "globs" field.
	Globs *string `json:"globs,omitempty"`
	// AppliesToDescription holds the value of the "applies_to_description" field.
	AppliesToDescription *string `json:"applies_to_description,omitempty"`
	// Severity holds the value of the "severity" field.
	Severity rule.Severity `json:"severity,omitempty"`
...
func (*Rule) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case rule.FieldTrustScore, rule.FieldConfidence:
			values[i] = new(sql.NullFloat64)
		case rule.FieldID, rule.FieldSourceKind, rule.FieldSourceRef, rule.FieldProjectID, rule.FieldRepoID, rule.FieldBody, rule.FieldActivation, rule.FieldGlobs, rule.FieldAppliesToDescription, rule.FieldSeverity, rule.FieldSupersededByID, rule.FieldCreatedByActorID:
			values[i] = new(sql.NullString)
		case rule.FieldCreatedAt, rule.FieldUpdatedAt, rule.FieldLastAccessedAt, rule.FieldLastValidatedAt, rule.FieldArchivedAt:
...
func (_m *Rule) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case rule.FieldLastValidatedAt:
			if value, ok := values[i].(*sql.NullTime); !ok {
		...
		case rule.FieldActivation:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field activation", values[i])
			} else if value.Valid {
				_m.Activation = rule.Activation(value.String)
			}
		case rule.FieldGlobs:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field globs", values[i])
			} else if value.Valid {
				_m.Globs = new(string)
				*_m.Globs = value.String
			}
		case rule.FieldAppliesToDescription:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field applies_to_description", values[i])
			} else if value.Valid {
				_m.AppliesToDescription = new(string)
				*_m.AppliesToDescription = value.String
			}
		case rule.FieldSeverity:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field severity", values[i])
			} else if value.Valid {
				_m.Severity = rule.Severity(value.String)
			}
		case rule.FieldSupersededByID:
			if value, ok := values[i].(*sql.NullString); !ok {
		...
		case rule.FieldCreatedByActorID:
			if value, ok := values[i].(*sql.NullString); !ok {
...
func (_m *Rule) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", _m.Activation))
	builder.WriteString(", ")
	if v := _m.Globs; v != nil {
	...
	builder.WriteString(", ")
	if v := _m.AppliesToDescription; v != nil {
		builder.WriteString("applies_to_description=")
		builder.WriteString(*v)
	}
	builder.WriteString(", ")
