...
## CLAUDE.md
Anthropic's convention for AI context files. aicoder's default render target. The AI reads this; the user writes to aicoder; lore render bridges the two.
...
## Prefix
The 3-character identifier prefix on every opaque ID. `mem_` = memory, `rul_` = rule, `dec_` = decision, etc. 32 registered prefixes; see `saas/pkg/aicoder/ids/registry.go`.

## Project
The top-level scope in aicoder. One project per `lore init`. Has many repos.
