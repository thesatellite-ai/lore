...
```bash
lore support-bundle --out=/tmp/aicoder-bug.tar.gz
...
mv .lore/lore.db /Users/me/local-aicoder/$(basename $PWD).db
mkdir -p .lore
ln -s /Users/me/local-aicoder/$(basename $PWD).db .lore/lore.db
# (NOTE: symlink will be refused — instead use --db flag or LORE_DB env.)
echo "LORE_DB=/Users/me/local-aicoder/$(basename $PWD).db" >> .envrc
```
