...
description: |
  ...
  • mentions aicoder, .lore/, CLAUDE.md, render, memory add, rule add
...
| Result | What to do |
|---|---|
| **yes** | This project uses aicoder. Every learned fact, rule, decision should be captured before the session ends. |
| **no, but the user wants to start** | Run `lore init --non-interactive` (see [P1 — Bootstrap](#workflow-p1)). |
...
| "**How do I** bootstrap aicoder?" / "**What's the procedure** for X?" | Answer with the command sequence. NO capture. |
...
### ⚠ Anti-pattern: do NOT roleplay aicoder's output
...
The user wants to start using aicoder. Use existing CLAUDE.md / AGENTS.md / .cursorrules as the seed. **Full recipe: [PLAYBOOKS.md § P1](./PLAYBOOKS.md), worked example: [examples/01-bootstrap.md](./examples/01-bootstrap.md).**
...
|---|---|---|
| Capturing in your reply but not calling `aicoder` | Dies with the session | `lore memory add` |
